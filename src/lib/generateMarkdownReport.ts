...
import { themePhrase, type DatasetUnit } from "./datasetInfo";
 ...
 *
 * `sentimentSource` names how the findings were classified, so an empty
...
export function generateMarkdownReport(
  ...
  // An empty section states WHY it is empty. "No issues met the threshold" is
  // only true when the engine could read the text at all; a rating-based engine
  // over product-average rows never could, and must not imply otherwise.
  const noActions =
    sentimentSource === "rating" && unit.isProductLevel
      ? "No actions were recommended. Actions are derived from faults, and this engine infers sentiment from the averaged product rating rather than the review text, so complaints written inside these records are not detected — a limit of the engine, not evidence there are none."
      : `No actions were recommended because no ${themePhrase(unit, "issues")} met the evidence threshold.`;
